...
<form action="{{route('consulta.update',['consultum'=>$consulta])}}" method="POST">
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="motivoConsulta" name = "motivoConsulta" placeholder="motivo de la consulta" required value={{$consulta->motivoConsulta}}>
    </div>
    <div class="form-group">
        <label for="expectativa">Expectativa </label>
        <input type="text" class="form-control" id="expectativa" name = "expectativa" placeholder="expectativas" required value = {{$consulta->expectativa}}>
    </div>
    <div class="form-group">
      ...
      <div class="form-group">
        ...
        @foreach ($pacientes as $paciente)
            ...
            <option selected value ={{$paciente->id}}  >{{$personas->where('id',$paciente->id)->first()->nombres . " " . $personas->where('id',$paciente->id)->first()->apellidos}}</option>
            @else
            <option value ={{$paciente->id}}  >{{$personas->where('id',$paciente->id)->first()->nombres . " " . $personas->where('id',$paciente->id)->first()->apellidos}}</option>
            @endif
